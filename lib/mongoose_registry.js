...
MongooseRegistry.prototype.add = function(tag, schema, fn) {
    if (!_.isUndefined(fn) && !_.isNull(fn) && _.isFunction(fn)) {
        if (!(this.schemas[tag] = schema)) {
            fn("Unable to register schema");
        } else {
            fn(null, schema);
        }
    } else {
...
MongooseRegistry.prototype.remove = function(tag, fn) {
        ...
        if (!_.isUndefined(fn) && !_.isNull(fn) && _.isFunction(fn)) {
            if (!(delete this.schemas[tag])) {
                fn("Unable to delete schema");
            } else {
                fn(null, deleted);
            }
        } else {
